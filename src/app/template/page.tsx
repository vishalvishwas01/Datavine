...
const Page = () => {
  ...
  useEffect(() => {
    setForm1Toggle(
      form1Fields.length === 0 && !form1Headline && !form1Description
    );
    setForm2Toggle(
      form2Fields.length === 0 && !form2Headline && !form2Description
    );
    setForm3Toggle(
      form3Fields.length === 0 && !form3Headline && !form3Description
    );
    setForm4Toggle(
      form4Fields.length === 0 && !form4Headline && !form4Description
    );
  }, [
    form1Fields,
    form2Fields,
    ...
    <>
      {toggle ? (
        ...
        <div className="border-4 border-gray-400 relative h-100 w-100 rounded-3xl flex justify-center items-center overflow-hidden ">
          ...
          <div
            className={`text-black z-20 uppercase text-3xl font-semibold mask-b-from-neutral-300`}
          >
            {formPath}
          </div>
        </div>
  ...
  return (
    <div className=" h-screen w-screen flex justify-center items-center gap-5 pt-20 px-2 lg:px-10 pb-10">
      <div className="w-full h-full gap-10 flex justify-center items-center flex-wrap overflow-auto py-2">
        <FormCard
          toggle={form1Toggle}
          setToggle={setForm1Toggle}
          formPath="form1"
        />
        <FormCard
          toggle={form2Toggle}
          setToggle={setForm2Toggle}
          formPath="form2"
        />
        <FormCard
          toggle={form3Toggle}
          setToggle={setForm3Toggle}
          formPath="form3"
        />
        <FormCard
          toggle={form4Toggle}
          setToggle={setForm4Toggle}
          formPath="form4"
        />
      </div>
    </div>
